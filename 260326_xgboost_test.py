...
class Config:
    ...
    RESULTS_DIR = "260326_Results_XGBoost_MultiExperiments"
    ...
    # XGBoost에 NaN을 그대로 맡기기 위해 스케일러 비활성화
    USE_SCALER = False
    ...
    # Ablation baseline과 동일한 전체 feature 사용
    MODES = [
...
def pad_or_truncate_sequence(seq: np.ndarray, target_len: int) -> np.ndarray:
    if seq.shape[0] < target_len:
        ...
        if seq.shape[0] == 0:
            return np.full((target_len, seq.shape[1]), np.nan, dtype=np.float32)
...
def get_raw_file_label(filename):
    if '-005' in filename:
        return 1
...
# PREPROCESSING
# =========================================================
def preprocess_subject_data(df, au_features, gaze_angle_features):
    df = df.copy()
    ...
    if all(col in df.columns for col in ['gaze_angle_x', 'gaze_angle_y']):
        ...
        amp_rad = np.sqrt(dx**2 + dy**2)
        df['gaze_amp'] = np.degrees(amp_rad).astype(np.float32)

        gaze_vel = df['gaze_amp'] / dt
        gaze_vel = gaze_vel.replace([np.inf, -np.inf], np.nan)
        df['gaze_vel'] = gaze_vel.astype(np.float32)

        d_vel = df['gaze_vel'].diff()
        gaze_acc = d_vel / dt
        gaze_acc = gaze_acc.replace([np.inf, -np.inf], np.nan)
    ...
    if not nd_data.empty:
        target_cols = [c for c in (au_features + gaze_angle_features) if c in df.columns]
        if target_cols:
...
# =========================================================
def create_windows_from_file_list(file_list, target_labels_map):
    ...
    for file in file_list:
        try:
            ...
            # 숫자형 변환 실패 -> NaN
            numeric_cols = [c for c in columns_to_load + ['timestamp'] if c in df.columns]
            for col in numeric_cols:
                df[col] = pd.to_numeric(df[col], errors='coerce')

            # 라벨 없는 행만 제거
            df = df.dropna(subset=['Distraction']).reset_index(drop=True)
            if len(df) == 0:
                continue
            ...
            if 'timestamp' in df.columns and len(df) > 1:
                ...
                if len(valid_ts) > 1:
                    duration = valid_ts.iloc[-1] - valid_ts.iloc[0]
                    actual_fps = ((len(valid_ts) - 1) / duration) if duration > 0 else Config.FPS
            ...
            labels = df['Distraction'].astype(int).values
            ...
            for i in range(0, len(df) - file_window_size + 1, file_step_size):
                ...
                X_au_list.append(
                    pad_or_truncate_sequence(window_df[au_features].values, target_time_steps)
                )
                X_pose_list.append(
                    pad_or_truncate_sequence(window_df[pose_features].values, target_time_steps)
                )
                X_vehicle_list.append(
                    pad_or_truncate_sequence(window_df[vehicle_features].values, target_time_steps)
                )
                X_gaze_list.append(
                    pad_or_truncate_sequence(window_df[gaze_total_features].values, target_time_steps)
                )
...
# =========================================================
def nan_safe_stat(arr, stat_fn, axis):
    with np.errstate(all='ignore'):
        out = stat_fn(arr, axis=axis)
...
def summarize_sequence_features(X_seq: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    ...
    X_seq: shape = (N, T, F)
    반환: shape = (N, F * 6)
          [mean, var, std, skewness, kurtosis, missing_ratio]
    """
    mean_feat = nan_safe_stat(X_seq, np.nanmean, axis=1)
    var_feat = nan_safe_stat(X_seq, np.nanvar, axis=1)
    std_feat = nan_safe_stat(X_seq, np.nanstd, axis=1)

    centered = X_seq - mean_feat[:, np.newaxis, :]
    with np.errstate(all='ignore'):
        skew_feat = np.nanmean(centered ** 3, axis=1) / (std_feat ** 3 + eps)
        kurt_feat = np.nanmean(centered ** 4, axis=1) / (std_feat ** 4 + eps)
    kurt_feat = kurt_feat - 3.0

    missing_ratio = np.mean(np.isnan(X_seq), axis=1).astype(np.float32)

    X_static = np.concatenate([
        mean_feat,
        var_feat,
        std_feat,
        skew_feat.astype(np.float32),
        kurt_feat.astype(np.float32),
        missing_ratio
...
def plot_kfold_summary(final_results, save_dir, experiment_name):
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, f"KFold_Summary_{sanitize_filename(experiment_name)}.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    for exp_name, exp_cfg in Config.EXPERIMENTS.items():
        ...
        LOG_FILE = os.path.join(exp_dir, "training_result_log_xgboost.txt")
        ...
        with open(LOG_FILE, 'w', encoding='utf-8') as f:
            ...
            f.write("NaN Handling    : Keep NaN for XGBoost\n")
            f.write(f"Classes: {class_names}\n")
            f.write(f"Modes: {Config.MODES}\n")
            f.write("=" * 80 + "\n")

        print("=" * 80)
        print(f"🚀 STARTING XGBOOST EXPERIMENT: {exp_name}")
        print("=" * 80)
        ...
        for f in all_files:
            raw_file_label = get_raw_file_label(f)
        ...
        file_labels = np.array(selected_file_labels)

        unique, counts = np.unique(file_labels, return_counts=True)
        print(f"\n[INFO] {exp_name} File Distribution")
        for u, c in zip(unique, counts):
            print(f"{class_names[u]}: {c}")

        if len(file_paths) == 0:
            print(f"[WARNING] {exp_name}: 사용할 파일이 없습니다. 건너뜁니다.")
            continue
        ...
        if len(unique_labels) < 2 or min_class_count < 2:
            print(f"[WARNING] {exp_name}: StratifiedKFold를 수행하기 위한 파일 수가 부족합니다. 건너뜁니다.")
            continue
        ...
        skf_files = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=Config.SEED)

        for fold_no, (train_f_idx, val_f_idx) in enumerate(skf_files.split(file_paths, file_labels), 1):
            print("\n" + "-" * 80)
            print(f"{exp_name} | Fold {fold_no}/{n_splits} Data Loading & Processing")
            print("-" * 80)
            ...
            X_tr_au_seq, X_tr_pose_seq, X_tr_veh_seq, X_tr_gaz_seq, y_tr_raw = create_windows_from_file_list(train_files, target_labels_map)

            print("[INFO] 검증용 파일 윈도우 생성 중...")
            X_val_au_seq, X_val_pose_seq, X_val_veh_seq, X_val_gaz_seq, y_val_raw = create_windows_from_file_list(val_files, target_labels_map)
            ...
            feat_blocks_tr = {
                ...
                'POSE': summarize_sequence_features(X_tr_pose_seq),
            ...
            feat_blocks_val = {
                ...
                'POSE': summarize_sequence_features(X_val_pose_seq),
                'VEHICLE': summarize_sequence_features(X_val_veh_seq),
                'GAZE': summarize_sequence_features(X_val_gaz_seq)
            }

            del X_tr_au_seq, X_tr_pose_seq, X_tr_veh_seq, X_tr_gaz_seq
            del X_val_au_seq, X_val_pose_seq, X_val_veh_seq, X_val_gaz_seq
            gc.collect()

            for mode in Config.MODES:
                mode_save_dir = os.path.join(exp_dir, sanitize_filename(mode))
        ...
        print("\n" + "=" * 80)
        print(f"📊 FINAL XGBOOST SUMMARY [{exp_name}]")
        print("=" * 80)
        write_log(LOG_FILE, "\n" + "=" * 80, print_console=False)
        write_log(LOG_FILE, f"FINAL XGBOOST SUMMARY [{exp_name}]", print_console=False)
        ...
        for mode in Config.MODES:
            ...
            if Config.SAVE_COMBINED_CM:
                ...
                plot_confusion_matrix(cm_all, class_names, cm_path, f'Total Combined CM - {mode}')

            if Config.SAVE_COMBINED_ROC:
                roc_path = os.path.join(mode_save_dir, f"Combined_ROC_{sanitize_filename(mode)}.png")
                plot_multiclass_roc_curve(y_true_all, y_proba_all, class_names, roc_path, f'Total Combined ROC - {mode}')

        plot_kfold_summary(final_results, exp_dir, exp_name)
        print(f"\n[INFO] 모든 결과가 저장되었습니다: {exp_dir}")