...
class Config:
    ...
    RESULTS_DIR = "260323_Results_KNN_MultiExperiment_au_headpose_removed"
...
def get_feature_groups():
    au_features = [
        'AU06_r', 'AU07_r', 'AU10_r', 'AU12_r',
        'AU14_r', 'AU25_r', 'AU26_r', 'AU45_r'
    ]
    pose_features = []
...
def create_windows_from_file_list(file_list, target_labels_map):
    ...
    columns_to_load = au_features + vehicle_features + gaze_raw_features + ['Distraction']
    final_features = au_features + vehicle_features + gaze_total_features

    X_au_list, X_vehicle_list, X_gaze_list = [], [], []
    y_list = []
...
def plot_kfold_summary(final_results, save_dir, experiment_name):
    valid_modes = [m for m in final_results.keys() if len(final_results[m]['acc']) > 0]
    if not valid_modes:
        return

    acc_means = [np.mean(final_results[m]['acc']) for m in valid_modes]
    acc_stds = [np.std(final_results[m]['acc']) for m in valid_modes]
    f1_means = [np.mean(final_results[m]['f1']) for m in valid_modes]
    f1_stds = [np.std(final_results[m]['f1']) for m in valid_modes]

    x = np.arange(len(valid_modes))
    width = 0.35

    fig_w = max(14, len(valid_modes) * 1.2)
    plt.figure(figsize=(fig_w, 7))

    plt.bar(x - width/2, acc_means, width, yerr=acc_stds, label='Accuracy',
            capsize=5, color='skyblue', edgecolor='black')
    plt.bar(x + width/2, f1_means, width, yerr=f1_stds, label='Macro F1',
    ...
    plt.xticks(x, valid_modes, rotation=45, ha='right')
    plt.ylim(0, 1.1)
    plt.legend(loc='lower right')
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, "KFold_Summary_KNN_FileBasedSplit.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    print("=" * 100)
    print("🚀 STARTING KNN MULTI-EXPERIMENT (FILE-BASED STRATIFIED SPLIT)")
    print("=" * 100)
    ...
    for exp_name, exp_cfg in Config.EXPERIMENTS.items():
        ...
        exp_result_dir = os.path.join(Config.RESULTS_DIR, sanitize_filename(exp_name))
        os.makedirs(exp_result_dir, exist_ok=True)

        log_file = os.path.join(exp_result_dir, f"training_result_log_{sanitize_filename(exp_name)}.txt")
        ...
        for f in all_files:
            ...
            # ND를 사용하지 않는 실험이면 기존처럼 해당 클래스 파일만 포함
            else:
                if raw_file_label in target_labels_map:
                    selected_file_paths.append(os.path.join(folder_path, f))
                    selected_file_labels.append(target_labels_map[raw_file_label])
        ...
        if len(file_paths) == 0:
            write_log(log_file, "[WARNING] 실험 대상 파일이 없습니다.", print_console=True)
            continue

        file_dist_msg = f"Filtered File Distribution: {format_class_distribution(file_labels, class_names)}"
        write_log(log_file, file_dist_msg)

        unique_labels, counts = np.unique(file_labels, return_counts=True)
        min_class_count = counts.min()

        if len(unique_labels) < 2 or min_class_count < 2:
            write_log(log_file, f"[WARNING] {exp_name}: StratifiedKFold를 수행하기 위한 파일 수가 부족합니다.")
            continue
        ...
        for fold_no, (train_f_idx, val_f_idx) in enumerate(skf_files.split(file_paths, file_labels), 1):
            ...
            X_tr_au_seq, X_tr_veh_seq, X_tr_gaz_seq, y_tr_raw = create_windows_from_file_list(
                train_files, target_labels_map
            )

            print("[INFO] 검증용 파일 윈도우 생성 중...")
            X_val_au_seq, X_val_veh_seq, X_val_gaz_seq, y_val_raw = create_windows_from_file_list(
                val_files, target_labels_map
            )
            ...
            del X_tr_au_seq, X_tr_veh_seq, X_tr_gaz_seq
            del X_val_au_seq, X_val_veh_seq, X_val_gaz_seq
            gc.collect()
            ...
            for mode in Config.MODES:
                mode_save_dir = os.path.join(exp_result_dir, sanitize_filename(mode))
                ...
                if len(y_tr_bal) == 0 or len(y_val_bal) == 0:
                    write_log(log_file, f"[WARNING] {mode} 언더샘플링 후 데이터가 없습니다.")
                    continue

                if len(np.unique(y_tr_bal)) < num_classes or len(np.unique(y_val_bal)) < num_classes:
                    write_log(log_file, f"[WARNING] {mode} 언더샘플링 후 일부 클래스가 없어 건너뜁니다.")
                    continue

                write_log(log_file, f"[{mode}] Train Distribution (balanced): {format_class_distribution(y_tr_bal, class_names)}")
                write_log(log_file, f"[{mode}] Val Distribution (balanced): {format_class_distribution(y_val_bal, class_names)}")
        ...
        # -------------------------------------------------
        print("\n" + "=" * 80)
        print(f"📊 FINAL KNN SUMMARY [{exp_name}]")
        print("=" * 80)
        write_log(log_file, "\n" + "=" * 80, print_console=False)
        write_log(log_file, f"FINAL KNN SUMMARY [{exp_name}]", print_console=False)
        ...
        for mode in Config.MODES:
            ...
            if Config.SAVE_COMBINED_CM:
                ...
                plot_confusion_matrix(cm_all, class_names, cm_path, f'Total Combined CM - {exp_name} - {mode}')

            if Config.SAVE_COMBINED_ROC:
                roc_path = os.path.join(mode_save_dir, f"Combined_ROC_{sanitize_filename(mode)}.png")
                plot_multiclass_roc_curve(y_true_all, y_proba_all, class_names, roc_path,
                                          f'Total Combined ROC - {exp_name} - {mode}')

        plot_kfold_summary(final_results, exp_result_dir, f"{exp_name} - KNN Multi-Class/File-Based Split")
