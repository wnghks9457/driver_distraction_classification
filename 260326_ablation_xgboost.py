...
class Config:
    ...
    RESULTS_DIR = "260326_Results_XGBoost_MultiExperiments_Ablation"
    ...
    # group ablation에서 몇 개 그룹을 동시에 제외할지
    # [1]   -> 1개 그룹 제외만
    # [1,2] -> 1개 + 2개 그룹 제외
    GROUP_ABLATION_SIZES = [2]
...
def build_ablation_configs():
    ...
    # -------------------------------------------------
    # group-wise ablation
    # 1개 제외, 2개 제외 등을 Config.GROUP_ABLATION_SIZES로 제어
    # -------------------------------------------------
    for r in Config.GROUP_ABLATION_SIZES:
        if r < 1 or r >= len(group_names):
            continue

        for removed_groups_tuple in combinations(group_names, r):
            removed_groups = list(removed_groups_tuple)
            selected_groups = [g for g in group_names if g not in removed_groups]

            selected_features = []
            for g in selected_groups:
                selected_features.extend(feature_groups[g])

            abl_name = f"group_minus_{'_'.join(removed_groups)}"
...
def evaluate_model_performance(model, X_val, y_val, class_names, fold_no, save_dir, mode_name, log_file):
    ...
    log_msg = (
        f"   [{mode_name}] Acc: {acc:.4f}, BAcc: {bacc:.4f}, Prec: {prec:.4f}, "
        f"Rec: {rec:.4f}, F1: {f1:.4f}, Spec: {spec:.4f}, AUC: {auc_score:.4f}"
    )
...
if __name__ == "__main__":
    ...
    for exp_name, exp_cfg in Config.EXPERIMENTS.items():
        ...
        for abl_name, abl_cfg in ablation_configs.items():
            ...
            removed_groups_cfg = abl_cfg.get('removed_groups', [])

            exp_dir = os.path.join(Config.RESULTS_DIR, exp_name, abl_name)
            ...
            with open(LOG_FILE, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Removed Groups: {removed_groups_cfg}\n")
            ...
            print(f"🧩 Removed Groups: {removed_groups_cfg}")
            ...
            print(f"\n[INFO] {exp_name} | {abl_name} File Distribution")
            for u, c in zip(unique, counts):
                print(f"{class_names[u]}: {c}")

            if len(file_paths) == 0:
                print(f"[WARNING] {exp_name} - {abl_name}: 사용할 파일이 없습니다. 건너뜁니다.")
                continue
            ...
            if len(unique_labels) < 2 or min_class_count < 2:
                print(f"[WARNING] {exp_name} - {abl_name}: StratifiedKFold를 수행하기 위한 파일 수가 부족합니다. 건너뜁니다.")
                continue
            ...
            for fold_no, (train_f_idx, val_f_idx) in enumerate(skf_files.split(file_paths, file_labels), 1):
                ...
                if len(y_tr_bal) == 0 or len(y_val_bal) == 0:
                    print(f"[WARNING] 언더샘플링 후 데이터가 없습니다.")
                    continue
            ...
            print("\n" + "=" * 80)
            print(f"📊 FINAL XGBOOST SUMMARY [{exp_name} | {abl_name}]")
            print("=" * 80)
            write_log(LOG_FILE, "\n" + "=" * 80, print_console=False)
            write_log(LOG_FILE, f"FINAL XGBOOST SUMMARY [{exp_name} | {abl_name}]", print_console=False)
            ...
            if all_predictions['y_true']:
                ...
                if Config.SAVE_COMBINED_CM:
                    ...
                    cm_path = os.path.join(exp_dir, f"Combined_CM_{sanitize_filename(abl_name)}.png")
                    plot_confusion_matrix(cm_all, class_names, cm_path, f'Total Combined CM - {abl_name}')

                if Config.SAVE_COMBINED_ROC:
                    roc_path = os.path.join(exp_dir, f"Combined_ROC_{sanitize_filename(abl_name)}.png")
                    plot_multiclass_roc_curve(y_true_all, y_proba_all, class_names, roc_path, f'Total Combined ROC - {abl_name}')
            ...
            if abl_name.startswith("group_minus_"):
                ...
                removed_groups = removed_name.split("_")
                removed_group = ",".join(removed_groups)
            ...
            summary_row = {
                ...
                'removed_groups': ",".join(removed_groups) if removed_groups else "none",
