...
class Config:
    ...
    # 3-Class 비교를 위한 단일 마스터 폴더 (실행 시 통계량에 따라 변경됨)
    MASTER_DIR_TEMPLATE = "260319_ANOVA_Simulation_Results_nd-cded-md_{metric}"
...
def draw_and_save_multiclass_boxplot(feature, set_name, group_dict, p_val, significant, plot_dir, best_dir, metric):
    ...
    plt.title(f"Welch ANOVA ({metric.upper()}) - {feature}")
...
if __name__ == "__main__":
    ...
    if total_subjects < 3:
        ...
        for metric in metrics_to_test:
            print(f"\n{'='*60}")
            print(f"🚀 [실험 시작] 통계 기준: {metric.upper()}")
            print(f"{'='*60}")
            ...
            # 매 통계 지표마다 동일한 피험자 분배를 유지하기 위해 시드 초기화 (선택적)
            random.seed(RANDOM_SEED)
            ...
            for i in range(1, TOTAL_ITERATIONS + 1):
                ...
                print(f"▶ [{metric.upper()} - Iter {i}/{TOTAL_ITERATIONS}] 분석 중...")
            ...
            if all_anova_results:
                print(f"\n[{metric.upper()} 결과 종합] 3-Class 통계적 유의성 요약 중...")
                ...
                summary_path = os.path.join(master_dir, f"Total_Significance_Summary_{metric.upper()}.csv")
