...
class Config:
    ...
    RESULTS_DIR = "Results_RF_MultiExperiment"
...
def plot_kfold_summary(final_results, save_dir, experiment_name):
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, "KFold_Summary_RF_FileBasedSplit.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    print("=" * 100)
    print("🚀 STARTING RANDOM FOREST MULTI-EXPERIMENT (FILE-BASED STRATIFIED SPLIT)")
    print("=" * 100)
    ...
    for exp_name, exp_cfg in Config.EXPERIMENTS.items():
        ...
        with open(log_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"RF Params       : {Config.RF_PARAMS}\n")
            f.write("=" * 80 + "\n")
        ...
        for fold_no, (train_f_idx, val_f_idx) in enumerate(skf_files.split(file_paths, file_labels), 1):
            print("\n" + "-" * 80)
            print(f"[{exp_name}] Fold {fold_no}/{Config.N_SPLITS} Data Loading & Processing")
            print("-" * 80)
            ...
            write_log(log_file, f"Train Raw Distribution: {format_class_distribution(y_tr_raw, class_names)}")
            write_log(log_file, f"Val Raw Distribution  : {format_class_distribution(y_val_raw, class_names)}")
            ...
            for mode in Config.MODES:
                ...
                write_log(log_file, f"[{mode}] Train Balanced Distribution: {format_class_distribution(y_tr_bal, class_names)}", print_console=False)
                write_log(log_file, f"[{mode}] Val Balanced Distribution  : {format_class_distribution(y_val_bal, class_names)}", print_console=False)
        ...
        print("\n" + "=" * 80)
        print(f"📊 FINAL RANDOM FOREST SUMMARY [{exp_name}]")
        print("=" * 80)
        write_log(log_file, "\n" + "=" * 80, print_console=False)
        write_log(log_file, f"FINAL RANDOM FOREST SUMMARY [{exp_name}]", print_console=False)
        ...
        for mode in Config.MODES:
            ...
            if Config.SAVE_COMBINED_ROC:
                ...
                plot_multiclass_roc_curve(y_true_all, y_proba_all, class_names, roc_path, f'Total Combined ROC - {exp_name} - {mode}')

        plot_kfold_summary(final_results, exp_result_dir, f"{exp_name} - Random Forest File-Based Split")
