...
class Config:
    ...
    RESULTS_DIR = "260324_Results_SVM_MultiExperiment_Ablation"
...
if __name__ == "__main__":
    ...
    print("=" * 100)
    print("🚀 STARTING SVM MULTI-EXPERIMENT ABLATION (FILE-BASED STRATIFIED SPLIT)")
    print("=" * 100)
    ...
    for exp_name, exp_cfg in Config.EXPERIMENTS.items():
        ...
        if len(file_paths) == 0:
            print(f"[WARNING] {exp_name}: 실험 대상 파일이 없습니다.")
            continue
        ...
        for abl_name, abl_cfg in ablation_configs.items():
            ...
            print("\n" + "=" * 100)
            print(f"🚀 STARTING SVM EXPERIMENT: {exp_name}")
            ...
            for fold_no, (train_f_idx, val_f_idx) in enumerate(skf_files.split(file_paths, file_labels), 1):
                ...
                write_log(log_file, f"Train Raw Distribution: {format_class_distribution(y_tr_raw, class_names)}", print_console=False)
                write_log(log_file, f"Val Raw Distribution  : {format_class_distribution(y_val_raw, class_names)}", print_console=False)
                ...
                if len(y_tr_bal) == 0 or len(y_val_bal) == 0:
                    print(f"[WARNING] {abl_name} 언더샘플링 후 데이터가 없습니다.")
                    continue

                write_log(log_file, f"Train Balanced Distribution: {format_class_distribution(y_tr_bal, class_names)}", print_console=False)
                write_log(log_file, f"Val Balanced Distribution  : {format_class_distribution(y_val_bal, class_names)}", print_console=False)
            ...
            print("\n" + "=" * 80)
            print(f"📊 FINAL SVM SUMMARY [{exp_name} | {abl_name}]")
            print("=" * 80)
            write_log(log_file, "\n" + "=" * 80, print_console=False)
            write_log(log_file, f"FINAL SVM SUMMARY [{exp_name} | {abl_name}]", print_console=False)
            ...
            pd.DataFrame(summary_results).to_csv(
                os.path.join(Config.RESULTS_DIR, "ablation_summary_svm.csv"),
                index=False,
                encoding='utf-8-sig'
            )
