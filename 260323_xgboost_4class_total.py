...
class Config:
    ...
    RESULTS_DIR = "260323_Results_XGBoost_MultiExperiments"
...
if __name__ == "__main__":
    ...
    for exp_name, exp_cfg in Config.EXPERIMENTS.items():
        ...
        # 각 실험의 타겟 클래스에 따라 파일을 선택하는 로직
        for f in all_files:
            raw_file_label = get_raw_file_label(f)

            # ND(0)를 사용하는 실험이면 모든 파일을 포함
        ...
        for fold_no, (train_f_idx, val_f_idx) in enumerate(skf_files.split(file_paths, file_labels), 1):
            ...
            X_tr_au_seq, X_tr_veh_seq, X_tr_gaz_seq, y_tr_raw = create_windows_from_file_list(train_files, target_labels_map)

            print("[INFO] 검증용 파일 윈도우 생성 중...")
            X_val_au_seq, X_val_veh_seq, X_val_gaz_seq, y_val_raw = create_windows_from_file_list(val_files, target_labels_map)
